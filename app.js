...
const BOOKINGS_KEY = "gp-plumbing-demo-bookings";
...
function saveAppointment(appointment) {
  const appointments = getSavedBookings();
  appointments.unshift({ ...appointment, id: crypto.randomUUID(), createdAt: new Date().toISOString() });
  localStorage.setItem(BOOKINGS_KEY, JSON.stringify(appointments.slice(0, 6)));
}
...
function renderSavedBookings() {
  const container = document.querySelector("#savedBookings");
  if (!container) return;
  ...
  if (!bookings.length) {
    ...
    empty.textContent = "No demo bookings yet. Submit the form to see confirmed visits here.";
  ...
  bookings.forEach((booking) => {
    ...
    card.className = "saved-booking-item";
    card.innerHTML = `
      <div>
...
function setupBookingForm() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    saveBookingState(appointment);
    renderSavedBookings();
    renderPreview();
    status.textContent = `Booking confirmed for ${appointment.date} during ${appointment.time}.`;
    form.reset();
    setMinDate();
  });
}
